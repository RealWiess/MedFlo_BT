...
class firmware(object):
    def __init__(self, cfg_file):
        ...
        self.ota_zip_dir = os.path.join(self.fw_dir, 'ota_zip')
        self.orig_bin_dir = self.bin_dir + '_orig'
        self.orig_ota_dir = self.ota_dir + '_orig'
        self.orig_ota_zip_dir = self.ota_zip_dir + '_orig'
        ...
        '''
        if not os.path.isdir(self.ota_zip_dir):
            os.mkdir(self.ota_zip_dir)
        if not os.path.isdir(self.orig_ota_zip_dir):
            os.mkdir(self.orig_ota_zip_dir)
...
#        print(self.partitions)
        ...
        param_file = os.path.join(self.bin_dir, 'param.bin')
        #self.generate_partition_table(param_file)
        self.generate_firmware_partition_table()
    ...
    def generate_firmware_partition_table(self):
        ...
        boot_file = ''
        for part in self.partitions:
            if ('BOOT' == part['type']) and ('0' == part['mirror_id']):
                boot_file = os.path.join(self.bin_dir, part['file_name']);
                break
    ...
    def generate_firmware_xml(self, firmware_xml): 
        root = ET.Element('firmware')
        root.text = '\n\t'
        root.tail = '\n'
    
        descirption = ET.SubElement(root, 'descirption')
        descirption.text = 'Firmware layout for tai'
        descirption.tail = '\n\t'

        disk_size = ET.SubElement(root, 'disk_size')
        disk_size.text='0x80000'
        disk_size.tail = '\n\t'

        partitions = ET.SubElement(root, 'partitions')
        partitions.text = '\n\t'
        partitions.tail = '\n'

        for part in self.partitions:
            if (part['name'] == 'nv_user'):
                continue
            part_node = ET.SubElement(partitions, 'partition')
            part_node.text = '\n\t\t'
            part_node.tail = '\n\t'
            ...
            name = ET.SubElement(part_node, 'name')
            name.text=part['name']
            name.tail = '\n\t\t'

            file = ET.SubElement(part_node, 'file')
            if ('file_name' in part.keys()):
                file.text = part['file_name']
            else:
                for part_tmp in self.partitions:
                    if (part_tmp['type'] == part['type'] and 'file_name' in part_tmp.keys()):
                        file.text = part_tmp['file_name']
                        break
            file.tail = '\n\t\t'

            address = ET.SubElement(part_node, 'address')
            address.text=part['address']
            address.tail = '\n\t\t'

            if ('true' == part['enable_crc']):
                enable_crc = ET.SubElement(part_node, 'enable_crc')
                enable_crc.text=part['enable_crc']
                enable_crc.tail = '\n\t\t'

            enable_randomize = ET.SubElement(part_node, 'enable_randomize')
            enable_randomize.text = part['enable_encryption']
            enable_randomize.tail = '\n\t\t'

            enable_ota = ET.SubElement(part_node, 'enable_ota')
            enable_ota.text = part['enable_ota']
            enable_ota.tail = '\n\t\t'

            enable_raw = ET.SubElement(part_node, 'enable_raw')
            enable_raw.text = part['enable_raw']
            enable_raw.tail = '\n\t\t'

            enable_dfu = ET.SubElement(part_node, 'enable_dfu')
            enable_dfu.text = part['enable_dfu']
            enable_dfu.tail = '\n\t\t\t'

            fw_id = ET.SubElement(part_node, 'fw_id')
            if (part['name'] == 'nv_user' or part['name'] == 'nv_factory'):
                fw_id.text = '255'
            else:
                fw_id.text = part['mirror_id']
            fw_id.tail = '\n\t\t'

        tree = ET.ElementTree(root)
        tree.write(os.path.join(self.bin_dir, firmware_xml), encoding='UTF-8')
        shutil.copyfile(os.path.join(self.bin_dir, firmware_xml), os.path.join(self.orig_bin_dir, firmware_xml))

    def generate_att_firmware(self, fw_name):
        print('FW: Build ATT firmware ' + fw_name)
        maker_cfgfile = os.path.join(self.bin_dir, 'fw_maker_att.cfg')
        att_filename = 'tai.fw'
        att_file = os.path.join(self.bin_dir, att_filename)
        ...
        if (is_windows()):
            ...
            if ('32bit' == platform.architecture()[0]):
                ...
                # linux x86_64
                maker_path = script_path + '/utils/linux-x86_64/maker/PyMaker.pyo'
            cmd = ['python2', '-O', maker_path, '-c', maker_cfgfile, '-o', att_file, '--mf', '1']
        ...
        curr_path = os.getcwd()
        if (is_windows()):
            att_maker_dir = script_path + '/utils/windows/att_maker'
            shutil.copyfile(os.path.join(att_maker_dir, 'config.txt'), os.path.join(self.bin_dir, 'config.txt'))
            shutil.copyfile(os.path.join(att_maker_dir, 'config.xml'), os.path.join(self.bin_dir, 'config.xml'))
            shutil.copyfile(os.path.join(att_maker_dir, 'acttest.ap'), os.path.join(self.bin_dir, 'acttest.ap'))
            self.generate_firmware_xml('firmware.xml');
            firmware_xml = os.path.join(self.bin_dir, 'firmware.xml')
            os.chdir(self.bin_dir)
            cmd = [script_path + '/utils/windows/att_maker/att_maker.exe', fw_name, '5', 'acttest.ap', 'config.xml', att_filename, 'firmware.xml', 'config.txt']
            print(cmd)
        else:
            panic('system platform not support!')
        ...
        print_notice('Build successfully!')
        shutil.move(os.path.join(self.bin_dir, fw_name), os.path.join(self.fw_dir, fw_name))
        if is_msys():
            print_notice('Firmware: ' + cygpath(fw_name))
        else:
            print_notice('Firmware: ' + os.path.join(self.fw_dir, fw_name))

        # remove tmp files in self.bin_dir
        os.remove(att_file)
        os.remove(maker_cfgfile)
        os.remove('base.fw')
        os.remove('att.ini')
        os.remove('upgrade.ini')
        os.remove('firmware.xml')
        os.remove('config.txt')
        os.remove('config.xml')
        os.remove('acttest.ap')

        # remove tmp files in self.orig_bin_dir
        os.remove(os.path.join(self.orig_bin_dir, 'firmware.xml'))
        os.remove(os.path.join(self.orig_bin_dir, 'att.ini'))
        os.remove(os.path.join(self.orig_bin_dir, 'base.fw'))
        os.remove(os.path.join(self.orig_bin_dir, 'upgrade.ini'))
        os.remove(os.path.join(self.orig_bin_dir, 'fw_maker_att.cfg'))
        os.chdir(curr_path)
    ...
    def generate_ota_zip_xml(self, ota_dir):
        ...
        for part in self.partitions:
            if ('true' == part['enable_ota']):
                ...
                if ('file_name' in part.keys()):
                    ...
                    for part_tmp in self.partitions:
                        if (part_tmp['type'] == part['type'] and 'file_name' in part_tmp.keys() and 'true' == part_tmp['enable_ota']):
                            #print('part type ' + part['type'] + ' assgin file_name ' + part_tmp['file_name'])
                            file.text = part_tmp['file_name']
                            partfile = os.path.join(ota_dir, part_tmp['file_name']);
                            break
                file.tail = '\n\t\t'
                ...
                type.text=part['mirror_id']
                type.tail = '\n\t\t'

                crc = ET.SubElement(part_node, 'crc32')
                crc.text = str(crc32_file(partfile))
                crc.tail = '\n\t'
        ...
        tree = ET.ElementTree(root)
        tree.write(os.path.join(ota_dir, 'Update.xml'), encoding='UTF-8')
    ...
    def generate_ota_image_zip_internal(self, ota_file, ota_dir = ''):
        if os.path.exists(ota_file):
            os.remove(ota_file)

        self.generate_ota_zip_xml(ota_dir)
        zip_dir(ota_dir, ota_file)
    ...
    def generate_ota_image_zip(self, ota_filename):
        print('FW: Build OTA zip \'' +  ota_filename + '\'')

        # generate OTA firmware with crc/randomizer
        self.copy_ota_files(self.bin_dir, self.ota_zip_dir)
        self.generate_ota_image_zip_internal(os.path.join(self.fw_dir, ota_filename), self.ota_zip_dir)

        '''
        # generate OTA firmware without crc/randomizer
        self.copy_ota_files(self.orig_bin_dir, self.orig_ota_zip_dir)
        self.generate_ota_image_zip_internal(os.path.join(self.fw_dir, \
                ota_filename[:-4] + '_orig' + ota_filename[-4:]), self.orig_ota_zip_dir)
        '''
    ...
    def generate_earphone_config(self, mcfg_file, upg_ini_file):
        with open(mcfg_file, 'w') as f, open(upg_ini_file, 'w') as f_u:
            ...
            f.write('BASEFILE="afi_earphone.bin";\n')
            ...
            for part in self.partitions:
                if (('file_name' in part.keys()) and ('true' == part['enable_dfu']) and ('BOOT' != part['type'])):
                    if ((0 == int(part['storage_id'], 0)) or (2 == int(part['storage_id'], 0))):
                        w_file_name = part['file_name']
                        w_file_addr = int(part['file_address'], 0)
                        f.write('WFILE="%s",0x%x;\n' %(w_file_name, w_file_addr))
                        swrite_data_list.append(("swrite 0x%08x 0 %s" %(w_file_addr, w_file_name), w_file_addr))

            # write external partition
            for part in self.partitions:
                if (('file_name' in part.keys()) and ('true' == part['enable_dfu']) and ('BOOT' != part['type'])):
                    if ((1 == int(part['storage_id'], 0)) or (3 == int(part['storage_id'], 0))):
                        w_file_name = part['file_name']
                        w_file_addr = int(part['file_address'], 0)
                        f.write('EXTERN_DATA_FILE="%s",0x%x;\n' %(w_file_name, w_file_addr))
                        swrite_data_list.append(("swrite 0x%08x 0 %s" %(w_file_addr, w_file_name), w_file_addr))
            ...
            swrite_data_list.sort(key = lambda x:x[1])
            f_u.write("\n".join([x[0] for x in swrite_data_list]))
            f_u.write("\ndisconnect\n")
            ...
            f.write('\n\nSETPATH=".\\adfu";\n')
            f.write('#include "fw_product.cfg"\n')
    ...
    def generate_earphone_firmware(self, fw_name):
        ...
        # prebuild_dir = "%s\\..\\..\\..\\prebuild" %self.fw_dir
        prebuild_dir = os.path.join(self.fw_dir, "..\\..\\..\\prebuild")
        ...
        if (is_windows()):
            if os.path.isdir(prebuild_dir):
                ...
                cmd = ['xcopy', prebuild_dir, self.bin_dir, '/F', '/S', '/y']
                run_cmd(cmd)
        ...
        self.build_atf_image(fw_file, self.att_dir)
...
def main(argv):
    ...
    try:
        ...
        if not fw.is_earphone_app:
            ...
            fw.generate_ota_image_zip(fw_prefix + '_ota.zip')

            #fw.generate_firmware(fw_prefix + '.fw')
            fw.generate_att_firmware(fw_prefix + '_atf.fw')
