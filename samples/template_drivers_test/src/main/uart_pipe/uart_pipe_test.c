...
#include "uart_pipe/uart_pipe_test.h"


/***************************************************
 * Attention: uart_pipe.c must add to project
 *
 * Brief:
...
void uart_pipe_test(void)
{
	int i;
	struct uart_data_buf *buf;

	/* if use uart1, disable jtag */
	if (!strcmp(CONFIG_UART_PIPE_ON_DEV_NAME, CONFIG_UART_1_NAME)) {
		sys_write32(sys_read32(JTAG_CTL) & ~(0x1<<31), JTAG_CTL);
	}
	
	if (!strcmp(CONFIG_UART_PIPE_ON_DEV_NAME, CONFIG_UART_CONSOLE_ON_DEV_NAME)) {
