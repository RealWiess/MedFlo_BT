...
/* the program mode used dma trans, the repeat and cntmax are in ram_buf */
typedef struct {
	u16_t *ram_buf;
	u32_t ram_buf_len;
	int (*program_callback)(void *cb_data, u8_t reason);
	void *cb_data;
	u8_t  multi_ch_en;/* <if or not enable other chans in the same group
					   * 0:not enable other chans in the same group, if not 0, one bit represents one channel, ie, bit0 enables pwm0/1's channel 0 > */
	u16_t use_cntmax_reg_subst_ram; /* if or not use CNTMAX reg substitute ram space.
									** 0: no use CNTMAX reg substitute ram(means cntmax transfer from ram)
									** > 0: use CNTMAX reg substitute ram and this value will be set to CNTMAX reg.
									*/
	u16_t use_repeat_reg_subst_ram; /* if or not use REPEAT reg substitute ram space.
									** 0: no use REPEAT reg substitute ram(means repeat transfer from ram)
									** > 0: use REPEAT reg substitute ram and this value will be set to REPEAT reg.
									*/
} pwm_program_ctrl_t;
...
*/
typedef struct {//second stage lead length and code
	u32_t ll;//length
	u16_t ld;//code
} pwm_ld_t;


typedef struct {//pwm dma cycle and repeat
	u16_t cycle;  // configed cycle
	u16_t repeat; // configed repeat
} pwm_dma_t;

struct ir_tx_data_param {
	u32_t *data;//ir send data
	u16_t mode;//ir protocol
	u16_t rate;// ir rate
	u8_t duty;//ir duty
	union {
		u32_t buf_num;//number of ir stage
		u32_t data_len;//pwm dma buffer data len
	};
	union {
		pwm_ld_t *lead;//second stage lead param
		pwm_dma_t dma;//pwm dma param
	};	
};

struct ir_tx_protocol_param {
	bool ir_lc_check_en;//leader code check enable
	u32_t ir_lc_bit_length;//leader code bit length
	u32_t ir_lc_code;//leader code
	u32_t ir_max_code;//the max ir code length

	bool ir_cv_check_en;//code value check enable
	u32_t ir_stop_bit;
	u32_t ir_dc_length;//data code length check
	u32_t code_bit_length;//code bit length
	u32_t ir_0_code;// 0 code
	u32_t ir_1_code;//1 code
	u32_t ir_tr0_code;//0 control code
	u32_t ir_tr1_code;//1 control code
	u32_t ir_trc_loc;//control code length

	bool ir_cr_check_en;//carrier wave rate check enable
	u8_t ir_cr_rate;//khz

	bool ir_Tf_check_en;//ir Tf check enable(Tf:Minimum cycle length)
	u32_t ir_Tf_length;//us/10(108ms -> 10800)

	bool ir_ic_check_en;//ir inverse code check enable
	u32_t ir_ic_mask;// ir code mask
	u8_t ir_ic_co;//ir code offset
	u8_t ir_ic_ico;//ir inverse code offset

	u8_t ir_asc;//cycle times

	short int ir_bias_val;// the bias of boundary between high volt and low volt in 1&0 code(us)
	u8_t ir_threshold_val;//The threshold value:ir_threshold_val/100
};

typedef struct {

	struct ir_tx_protocol_param *protocol;

	struct ir_tx_data_param *data;
...
/**
 * @typedef pwm_ir_stop_transfer_t
 * @brief Callback API upon setting pwm ir mode.
 * See @a pwm_ir_transfer_mode() for argument description
 */
typedef int (*pwm_ir_stop_transfer_t)(const struct device *dev);

#ifdef CONFIG_KEYPAD_SUPPORT_PRO_STUCK_KEY
typedef void (*pwm_ir_tx_stop_t)(const struct device *dev);
#endif
...
/**
 * @typedef pwm_pin_repeat_t
 * @brief Callback API upon stopping the specified pwm.
 * See @a pwm_pin_repeat_t() for argument description
 */
typedef int (*pwm_pin_repeat_t)(const struct device *dev, u32_t pwm, u8_t repeat);

/**
 * @typedef pwm_ir_tx_power_set_t
 * @brief Callback API upon stopping the specified pwm.
 * See @a pwm_ir_tx_power_set_t() for argument description
 */
typedef int (*pwm_ir_tx_power_set_t)(const struct device *dev, u8_t level);
...
__subsystem struct pwm_driver_api {
	...
	pwm_ir_stop_transfer_t ir_stop_transfer;
	pwm_pin_stop_t pin_stop;

	pwm_pin_repeat_t pin_repeat;

	pwm_ir_tx_power_set_t ir_tx_power_set;
 ...
 * @retval Negative errno code if failure.
 */
static inline int pwm_tx_power_set(const struct device *dev, u8_t level)
{
	struct pwm_driver_api *api;

	api = (struct pwm_driver_api *)dev->api;
	return api->ir_tx_power_set(dev, level);
}
...
static inline int pwm_ir_stop_transfer(const struct device *dev)
{
	struct pwm_driver_api *api;

	api = (struct pwm_driver_api *)dev->api;
	return api->ir_stop_transfer(dev);
}

#ifdef CONFIG_KEYPAD_SUPPORT_PRO_STUCK_KEY
static inline void pwm_ir_tx_stop(const struct device *dev)
{
	struct pwm_driver_api *api;

	api = (struct pwm_driver_api *)dev->api;
	return api->ir_tx_stop(dev);
}
#endif
...
/**
 * @brief repeat the PWM output by the specified pwm pin
 * @note This API cooperates with pwm_set_program_mode() function.
 *
 * @param dev Pointer to the device structure for the driver instance.
 * @param pwm PWM pin.
 ...
 * @retval Negative errno code if failure.
 */
static inline int pwm_pin_repeat(const struct device *dev, u32_t pwm, u8_t repeat)
{
	struct pwm_driver_api *api;

	api = (struct pwm_driver_api *)dev->api;
	return api->pin_repeat(dev, pwm, repeat);
}
