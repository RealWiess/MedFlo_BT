		...
		struct{
			union {
				struct{
					...
					uint16_t *data;/*!< ir data */
					uint16_t carry_rate;/* carry rate */
					uint8_t carry_duty;/* carry rate */
					bool abnormal_inter;/* Whether abnormal interruption occurs */
				}protocol;
				struct{
					uint16_t mode;/* protocol kind */
...
struct input_dev_driver_api {
	void (*set_param)(const struct device *dev, u32_t *conf_param);
...
/**
 * @brief Function to set params of specified input device.
 *
 * @param dev Pointer to the device structure for the input driver instance.
 *
 * @return none
 */
static inline void input_dev_set_param(const struct device *dev, u32_t *conf_param)
{
	const struct input_dev_driver_api *api = dev->api;

	if (api->set_param)
		api->set_param(dev, conf_param);
}
