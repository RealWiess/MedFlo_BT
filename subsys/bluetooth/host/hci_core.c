			 ...
			 struct net_buf **rsp)
{
	...
	if (err == (-EAGAIN)) {
		...
		BT_INFO("buf %s", bt_hex(buf->data, buf->len));
...
static void kernel_stack_dump(const struct k_thread *thread, void *user_data)
{
	unsigned int pcnt;
	size_t unused;
	...
	if (ret) {
		LOG_ERR("Unable to determine unused stack size (%d)\n", ret);
		return;
	}
	...
	LOG_INF("%p %-10s (real size %-4u): unused %-4u usage %u/%u (%u %%)",
		      thread,
		      tname ? tname : "NA",
...
static void hci_disconn_complete(struct net_buf *buf)
{
	...
	LOG_INF("************show kernel stacks************");
	hci_disconn_complete_prio(buf);

	k_thread_foreach(kernel_stack_dump, NULL);
...
int bt_addr_res_enable(uint8_t enable)
{
	return addr_res_enable(enable);
}
...
int hci_vs_set_local_feat(local_feat_t *feat)
{
	...
	buf = bt_hci_cmd_create(BT_HCI_OP_VS_SET_LOCAL_FEAT,
				sizeof(local_feat_t));
	if (!buf) {
		return -ENOBUFS;
	}
	...
	err = bt_hci_cmd_send_sync(BT_HCI_OP_VS_SET_LOCAL_FEAT, buf,
				   NULL);
	if (err) {
		BT_ERR("set local feature failed %d\n", err);
		return err;
	}
	return 0;
}

void hci_set_le_local_feat(uint8_t feat, uint8_t flag)
{
	struct net_buf *rsp;
	int err;
...
int hci_vs_ext_op_flags_cmd(uint32_t flags, bool en)
{
	struct set_op_flags *p;
	struct net_buf *buf;
	int err;

	buf = bt_hci_cmd_create(BT_HCI_OP_VS_SET_OP_FLAGS,
				sizeof(struct set_op_flags));
	if (!buf) {
		return -ENOBUFS;
	}

	p = (struct set_op_flags *)net_buf_add(buf, sizeof(struct set_op_flags));

	p->flags = flags;
	p->en = en;

	err = bt_hci_cmd_send_sync(BT_HCI_OP_VS_SET_OP_FLAGS, buf, NULL);
	if (err) {
		BT_ERR("set op flags failed %d\n", err);
		return err;
	}
	return 0;
}

void disable_slave_latency(struct bt_conn *conn)
{
	hci_vs_ext_op_flags_cmd(LL_OP_MODE_FLAG_ENA_SLV_LATENCY, 0);
}

void enable_slave_latency(struct bt_conn *conn)
{
	hci_vs_ext_op_flags_cmd(LL_OP_MODE_FLAG_ENA_SLV_LATENCY, 1);
}

int bt_hci_read_rssi(struct bt_conn *conn, int8_t *rssi)
{
	...
	*rssi = rp->rssi;
	...
	BT_DBG("status %d, handle %d, rssi %d", rp->status, rp->handle, rp->rssi);
